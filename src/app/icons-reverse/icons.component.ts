...
@Component({
  selector: 'app-icons-reverse',
  standalone: true,
  templateUrl: './icons.component.html',
  styleUrls: ['./icons.component.css'],
})
export class IconsReverseComponent implements AfterViewInit {
  @ViewChild('marqueeContent', { static: false }) marqueeContent:
    | ElementRef
    | undefined;
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      ...
      if (this.marqueeContent) {
        const marqueeContentElement = this.marqueeContent
        ...
        for (let i = 0; i < marqueeElementsDisplayed; i++) {
          const clone: Node =
            marqueeContentElement.children[i % totalChildren].cloneNode(true);
